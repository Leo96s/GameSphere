    ...
    <div class="w-full max-w-md">
      <Card class="overflow-hidden shadow-lg">
        ...
        <div class="px-6 py-8 text-center">
          <p class="text-gray-500 mb-2 font-medium">Recover Account</p>
...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    await requestAccountRecovery(values.email)

    // Store the email for use in the next step (recover account)
    localStorage.setItem("accountRecoveryEmail", JSON.stringify(values.email))

    success("Code sent", "Check your inbox")

    setTimeout(() => {
      router.push("/recoverAccount/recover")
    }, 1000)
